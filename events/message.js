...
module.exports = {
  ...
  if (!ayarlar.prefix.some(x => message.content.startsWith(x))) return;
  
  let command = message.content.split(' ')[0].slice(ayarlar.prefix.some(x => x.length)) || message.content.split(' ')[1]
